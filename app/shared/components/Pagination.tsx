...
export interface PaginationProps {
  ...
  itemLabel?: string
  showingText?: React.ReactNode | ((info: { start: number; end: number; total: number }) => React.ReactNode)
  className?: string
  disabled?: boolean
}
...
export default function Pagination({
  ...
  const renderShowingSummary = () => {
    if (typeof showingText === 'function') {
      return showingText({ start: startItem, end: endItem, total: totalElements })
    }
    if (showingText) {
      return showingText
    }
    if (itemLabel) {
      return t('pagination.showingWithLabel', {
        start: startItem,
        end: endItem,
        total: totalElements,
        label: itemLabel,
        defaultValue: `Showing ${startItem} to ${endItem} of ${totalElements} ${itemLabel}`
      })
    }
    return t('pagination.showing', {
      ...
      total: totalElements,
      defaultValue: `Showing ${startItem} to ${endItem} of ${totalElements} items`
    })
  }

  return (
    <div className={cn('flex flex-col sm:flex-row items-center justify-between gap-4 py-3 px-2', className)}>
      ...
      <div className='flex flex-wrap items-center gap-3 text-xs sm:text-sm text-gray-500 dark:text-gray-400'>
        <span>{renderShowingSummary()}</span>
          ...
          <PaginationContent>
            <PaginationItem>
              <PaginationPrevious
                onClick={(e) => {
                  e.preventDefault()
                  if (pageNumber > 1 && !disabled) onPageChange(pageNumber - 1)
                }}
            ...
            <PaginationItem>
              <PaginationNext
                onClick={(e) => {
                  e.preventDefault()
                  if (pageNumber < totalPages && !disabled) onPageChange(pageNumber + 1)
                }}
