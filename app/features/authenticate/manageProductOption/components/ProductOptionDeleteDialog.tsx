...
interface ProductOptionDeleteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  optionToDelete: ProductOptionResponse | null
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              <DialogTitle className='text-lg'>
                {t('productOption.deleteTitle')}
              </DialogTitle>
              <DialogDescription className='text-xs text-muted-foreground mt-0.5'>
                {t('productOption.cannotBeUndone')}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        {optionToDelete && (
          <div className='py-2 flex items-center gap-3 bg-gray-50 dark:bg-zinc-800/60 p-3 rounded-lg border border-gray-100 dark:border-zinc-800'>
            <p className='text-sm text-gray-700 dark:text-gray-300'>
              {t('productOption.deleteConfirmation', { name: optionToDelete.name })}
            </p>
          </div>
        )}
