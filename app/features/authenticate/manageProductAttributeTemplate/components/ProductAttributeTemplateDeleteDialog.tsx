...
import type { ProductAttributeTemplateItem } from '~/shared/types'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
interface ProductAttributeTemplateDeleteDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  templateToDelete: ProductAttributeTemplateItem | null
      ...
      <DialogContent className='sm:max-w-[400px]'>
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              <DialogTitle className='text-lg'>
                {t('productAttributeTemplate.deleteTitle')}
              </DialogTitle>
              <DialogDescription className='text-xs text-muted-foreground mt-0.5'>
                {t('productAttributeTemplate.cannotBeUndone')}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        {templateToDelete && (
          <div className='py-2 flex items-center gap-3 bg-gray-50 dark:bg-zinc-800/60 p-3 rounded-lg border border-gray-100 dark:border-zinc-800'>
            <p className='text-sm text-gray-700 dark:text-gray-300'>
              {t('productAttributeTemplate.deleteConfirmation', { name: templateToDelete.name })}
            </p>
          </div>
        )}
        ...
        <DialogFooter className='gap-2 sm:gap-0'>
          ...
          >
            ...
            {t('productAttributeTemplate.deleteTemplate')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
