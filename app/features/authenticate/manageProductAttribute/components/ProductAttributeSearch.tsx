...
export default function ProductAttributeSearch({
  ...
  isLoading = false,
  placeholder
}: ProductAttributeSearchProps) {
  const { t } = useTranslation()
  const [internalValue, setInternalValue] = useState(value)

  const searchPlaceholder = placeholder || t('productAttribute.searchPlaceholder')
    ...
    <div className='relative flex items-center w-full max-w-md'>
      ...
      {internalValue && (
        <Button
          ...
          title={t('productAttribute.clearSearch')}
        >
          <X className='size-3.5' />
          <span className='sr-only'>{t('productAttribute.clearSearch')}</span>
        </Button>
      )}
    </div>
  )
